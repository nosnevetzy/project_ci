...
class Refractions_Model extends MY_Model {
    ...
    function getFields($id) {
        ...
        self::_join();
        self::_fix_arg();
        parent::where(array('r.id_refraction' => $id));
        $query = $this->db->get();
    ...
    function getListPatient($id) {
        ...
        self::_join();
        self::_fix_arg();
        parent::where(array('r.patient_id' => $id));
        $query = $this->db->get();
    ...
    function getListLimit($where = array(), $where_string = '', $order_by = array(), $page = 0, $number = 0) {
        ...
        // parent::group_by("r.id_refraction");
        parent::orderby($order_by);
    ...
    private function _from() {
        $this->db->from("opt_refractions r");
    }
    ...
    private function _fix_arg() {
        $this->db->where(array('r.enabled' => 1));
    }
    ...
    function insert_table($data) {
        $this->db->insert("opt_refractions", $data);
    ...
    function insert_batch_table($data) {
        $this->db->insert_batch("opt_refractions", $data);
    }
    ...
    function update_table($data, $where) {
        self::where($where);
        $this->db->update("opt_refractions", $data);
    }
    ...
    function update_batch_table($data, $table_col) {
        $this->db->update_batch("opt_refractions", $data, $table_col);
    }
    ...
    function delete_table($table_col, $key) {
        $this->db->delete("opt_refractions", array($table_col => $key));
    }

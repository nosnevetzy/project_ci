...
class User extends Admin_Core {
    ...
    function add_user_page() {
        $data = array(
            'template' => parent::main_template(),
            'user_types' => $this->user_types_model->getSearch("", "", array("t.user_type" => "ASC"), true),
            'departments' => $this->departments_model->getSearch("", "", array("t.department_name" => "ASC"), true),
        );
    ...
    function access_user_page() {
        $data = array(
            'template' => parent::main_template(),
            'user_types' => $this->user_types_model->getSearch(array('ut.id_user_type !=' => 1), "", array("t.user_type" => "ASC"), true),
    ...
    function _method_delete_user() {
        if (IS_AJAX) {
            ...
            if ($row) {
                ...
                $arr = array(
                    'message_alert' => "Successfully deleted user $row->user_fname $row->user_lname",
            ...
            if ($row) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($count == 0) {
                        ...
                        $arr = array(
                            'message_alert' => "Successfully updated user " . $_POST['fname'] . " " . $_POST['lname'],
                            'redirect' => $_POST['redirect'],
                            'status' => "success",
                            'id' => $id_user
                        );
...
//                    copy(avatars_dir('profile.jpg'), $path['profile_thumb'] . "profile.jpg");
                    ...
                    $arr = array(
                        'message_alert' => "Code or Email already exist.",
                        'redirect' => $_POST['redirect'],
                        'status' => "error",
                        'id' => 0
                    );
    ...
    function _method_list_user() {
        ...
        $data = array(
            'functionName' => $this->function_name, // use to call functions for access
            'search' => $this->tools->getPost('search'),
            'sort' => $this->tools->getPost('sort'),
            'data_link' => (IS_AJAX) ? 1 : 0,
        );
        ...
        $this->load->view(admin_dir('lists/user/user'), $details);
    }
        ...
        if (IS_AJAX) {
            ...
            if ($row) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $count = $this->user_types_model->getSearch(array("ut.id_user_type !=" => $id_user_type, "t.user_type like" => $_POST['name']), "", "", "", true);
                    if ($count == 0) {
                        $datas = array(
    ...
    function _method_add_usertype() {
        if (IS_AJAX) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $count = $this->user_types_model->getSearch(array("t.user_type like" => $_POST['name']), "", "", "", true);
                if ($count == 0) {
                    $datas = array(
                        'user_type' => $_POST['name'],
                        'enabled' => 1,

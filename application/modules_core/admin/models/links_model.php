...
class Links_Model extends MY_Model {
    ...
    function getFields($id) {
        ...
        self::_join();
        self::_fix_arg();
        parent::where(array('l.id_link' => $id));
        $query = $this->db->get();
    ...
    private function _select() {
        $this->db->select("
			l.*,
			p_l.link_url as parent_link_url, p_l.link_name as parent_link_name, p_l.link_title as parent_link_title, p_l.link_icon  as parent_link_icon,
    ...
    private function _join() {
        $this->db->join('default_links p_l', 'p_l.id_link = l.parent_link_id', 'left');
        $this->db->join('default_user_types ut', 'ut.id_user_type = l.user_type_id', 'left');
        $this->db->join('default_users au', 'au.id_user = l.added_by', 'left');
        $this->db->join('default_users uu', 'uu.id_user = l.updated_by', 'left');
    }
    ...
    function insert_table($data) {
        $this->db->insert("default_links", $data);
    ...
    function insert_batch_table($data) {
        $this->db->insert_batch("default_links", $data);
    }
    ...
    function update_table($data, $where) {
        self::where($where);
        $this->db->update("default_links", $data);
    }
    ...
    function update_batch_table($data, $table_col) {
        $this->db->update_batch("default_links", $data, $table_col);
    }
    ...
    function delete_table($table_col, $key) {
        $this->db->delete("default_links", array($table_col => $key));
    }
